...
for whl in Path(".").glob("pkg/*.whl"):
    ...
    if 'mvp' not in sys.argv:
        # those are for index.json not index-bi
        if whlname.find('wasm32_mvp_emscripten')>0:
            continue

    # only scan pure and maybe mvp
    if whlname.find('wasm32_bi_emscripten')>0:
        continue
...
for whl in Path(".").glob("pkg/*wasm32_bi_emscripten.whl"):
    ...
    if not whlname.find('-abi3-')>0:
        ...
        # pre 0.9 only do 3.11 and no mvp
        if whlname.find('-cp312')>0:
            continue

        if whlname.find('-cp313')>0:
            continue
